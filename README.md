...
Bot Telegram yang terhubung ke AI (OpenRouter API) dengan memory percakapan per chat, error handling resilient, limit pesan per chat, dan dashboard web untuk pemantauan statistik.
...
- **Database:** Vercel KV (Redis) untuk statistik & riwayat chat
...
#### 2.3 Buat KV Store (Redis)
- Di dashboard Vercel → project → tab **Storage**
- Klik **Create Database** → pilih **KV (Marketplace/KV)** → create
- Connect KV store ke project
- Vercel akan attach secara otomatis (tidak perlu kode tambahan)

#### 2.4 Redeploy
Setelah KV terhubung, jalankan:
```bash
npx vercel --prod
```
...
- Jika gagal load, periksa browser console → likely error environment variable atau KV.
...
| Error 500 di dashboard Vercel | Environment variable hilang atau KV tidak connect | Tambah `TELEGRAM_TOKEN`, `OPENROUTER_API_KEY`, `WEBHOOK_SECRET`; redeploy |
...
| Memori tidak jalan (bot jawab acak) | Serverless function pakai KV lokal bukan global Vercel KV | Pastikan KV store terhubung di dashboard Vercel |
...
- Dikirim ke Vercel KV key `total_usage` (increment tiap pesan)
...
- [ ] API Key OpenRouter
- [ ] Akun Vercel + project
- [ ] KV store di Vercel connect
- [ ] `TELEGRAM_TOKEN`, `OPENROUTER_API_KEY`, `WEBHOOK_SECRET` di environment Vercel
